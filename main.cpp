...
static void addAll(skWindow* win, const std::shared_ptr<skSizer>& sizer) {
    for (auto& e : sizer->children()) win->addWidget(e.widget);
}
static void addAllFlex(skWindow* win, const std::shared_ptr<skFlexSizer>& sizer) {
    for (auto& e : sizer->children()) win->addWidget(e.widget);
}

int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int cmdShow) {
    auto app    = skApp::make(hInstance, cmdShow);
    auto window = app->createWindow("skWidgets Demo", 900, 700);
    if (!window) return 1;
    ...
    // HEADER
    // =========================================================================
    auto drawerBtn = skButton::make(6, 38, 32, 22, "\xe2\x89\xa1")
        ->onClick([drawer, hwnd]() { drawer->open(); InvalidateRect(hwnd, nullptr, FALSE); });
    drawerBtn->setTooltip("Open navigation drawer");

    auto ghLink = skLink::make(450, 87, "Source on GitHub \xe2\x86\x97", 11.f)
        ->onClick([window]() { window->showToast("Opening GitHub \xe2\x80\x94 imagine a browser launched!"); });
    ...
    auto darkChk = skCheckBox::make(660, 50, 124, 28, "Dark mode")
        ->onChange([hwnd](bool dark) {
    ...
    window->addWidget(drawerBtn)
          ->addWidget(skLabel::make(48, 46, 280, 36, "skWidgets", 28.f))
          ->addWidget(std::make_shared<skBadge>(208, 58, "alpha"))
          ->addWidget(skLabel::make(48, 84, 380, 18, "A Skia + Win32 UI framework", 12.f))
          ->addWidget(ghLink)
          ->addWidget(chipDesign)
          ->addWidget(chipCpp)
          ->addWidget(chipSkia)
          ->addWidget(av1)
          ->addWidget(av2)
          ...
          ->addWidget(headerImg)
          ->addWidget(darkChk)
          ->addWidget(std::make_shared<skCard>(24, 110, 852, 556));
    ...
    auto volLabel = skLabel::make(0, 0, 320, 18, "Volume: 65%", 12.f);
    auto progBar  = skProgressBar::make(0, 0, 320, 10)->withValue(0.65f);
    auto slider   = skSlider::make(0, 0, 320, 26)
        ->withValue(0.65f)
        ->onChange([volLabel, progBar, hwnd](float v) {
    ...
    auto saveBtn   = skButton::make(0, 0, 110, 38, "Save");
    auto cancelBtn = skButton::make(0, 0, 110, 38, "Cancel");
    saveBtn->setTooltip("Save your changes");
    cancelBtn->setTooltip("Close");

    auto col = skSizer::make(skDirection::Column, 12)
        ->add(skLabel::make(0, 0, 320, 18, "Your name", 12.f), 18)
        ->add(nameInput, 36)
        ->add(volLabel, 18)
        ->add(progBar, 10)
        ->add(slider, 26)
        ->add(skLabel::make(0, 0, 320, 18, "Notifications", 12.f), 18)
        ->add(rb1, 28)
        ->add(rb2, 28)
        ->add(rb3, 28)
        ->add(skLabel::make(0, 0, 320, 18, "Priority (1-10)", 12.f), 18)
        ->add(prioInput, 34)
        ->add(saveBtn, 38);
    ...
    auto spinner = skSpinner::make(
        cancelBtn->x + cancelBtn->w + 12,
        cancelBtn->y + (cancelBtn->h - 28) / 2, 28);

    addAll(window, col);
    window->addWidget(cancelBtn)->addWidget(spinner);
    ...
    auto tabBar = skTabBar::make(kTabX, kTabY, kTabW, 34);
    ...
    std::shared_ptr<skDropdown> dropdown;
    {
        ...
        auto chk3 = skCheckBox::make(0, 0, kTabW, 28, "Send usage statistics");

        dropdown = skDropdown::make(0, 0, kTabW, 36);
        ...
        auto s = skSizer::make(skDirection::Column, 10)
            ->add(tog1, 28)
            ->add(tog2, 28)
            ->add(chk3, 28)
            ->add(skLabel::make(0, 0, kTabW, 18, "Accent colour", 12.f), 18)
            ->add(dropdown, 36)
            ->add(std::make_shared<skSeparator>(0, 0, kTabW, 1), 1)
            ->add(skLabel::make(0, 0, kTabW, 18, "Account password", 12.f), 18)
            ->add(pasInp, 36);
    ...
    dropdown->onChange([hwnd](int idx, const std::string&) {
        auto th = skGetTheme();
        switch (idx) {
    ...
    std::vector<std::shared_ptr<skWidget>> filesW;
    {
        ...
        auto openFileBtn = skButton::make(kTabX, kContentY + 408, 110, 26, "Browse files")
            ->onClick([fileDlg, editor, topRow, hwnd]() {
        ...
        auto fileLbl = skLabel::make(kTabX + 118, kContentY + 412, kTabW - 118, 18,
    ...
    std::vector<std::shared_ptr<skWidget>> notesW;
    {
        ...
        auto clearBtn = skButton::make(kTabX, kContentY + 356, 80, 26, "Clear log")
            ->onClick([console, hwnd]() { console->clear(); InvalidateRect(hwnd, nullptr, FALSE); });

        auto notesLbl = skLabel::make(kTabX, kContentY,       kTabW, 18, "Notes",     12.f);
        auto textArea = std::make_shared<skTextArea>(kTabX, kContentY + 22, kTabW, 116, "Type here...");
        auto conLbl   = skLabel::make(kTabX, kContentY + 146, kTabW, 18, "Event log", 12.f);

        for (auto& w : { std::static_pointer_cast<skWidget>(notesLbl),
                          std::static_pointer_cast<skWidget>(textArea),
                          std::static_pointer_cast<skWidget>(conLbl),
                          std::static_pointer_cast<skWidget>(console),
                          std::static_pointer_cast<skWidget>(clearBtn) }) {
            window->addWidget(w); notesW.push_back(w);
        }
    }
    ...
    std::vector<std::shared_ptr<skWidget>> aboutW;
    {
        ...
        int cy = 8;
        for (auto* txt : items) { exp1->addChild(skLabel::make(12, cy, kTabW - 24, 16, txt, 11.f)); cy += 20; }
    ...
    std::vector<std::shared_ptr<skWidget>> galleryW;
    {
        ...
        // Chips row
        auto chipLbl = skLabel::make(kTabX, kContentY + 32, 40, 16, "Chips", 11.f);
        window->addWidget(chipLbl); galleryW.push_back(chipLbl);

        auto gc1 = std::make_shared<skChip>(kTabX + 50, kContentY + 30, "Design");
        auto gc2 = std::make_shared<skChip>(0,           kContentY + 30, "C++");
        auto gc3 = std::make_shared<skChip>(0,           kContentY + 30, "Skia");
        gc1->setOnClick([window]() { window->showToast("Design"); });
        gc2->setOnClick([window]() { window->showToast("C++");    });
        gc3->setOnClick([window]() { window->showToast("Skia");   });
        gc2->x = gc1->x + gc1->w + 8;
        gc3->x = gc2->x + gc2->w + 8;
        for (auto& c : { gc1, gc2, gc3 }) { window->addWidget(c); galleryW.push_back(c); }

        // Avatars row
        auto avLbl = skLabel::make(kTabX + 260, kContentY + 32, 50, 16, "Avatars", 11.f);
        ...
        // Navigation Rail
        auto navContent = skLabel::make(kTabX + 76, kContentY + 128, kTabW - 76 - 20, 40, "Home", 13.f);
        ...
        navRail->setOnChange([navContent, hwnd](int, const std::string& lbl) {
            navContent->setText(lbl); InvalidateRect(hwnd, nullptr, FALSE);
        });

        auto navLbl = skLabel::make(kTabX, kContentY + 90, 80, 16, "Nav Rail", 11.f);
        ...
        auto overlayLbl = skLabel::make(kTabX, kContentY + 244, kTabW, 16, "Overlays", 11.f);
        window->addWidget(overlayLbl); galleryW.push_back(overlayLbl);

        auto showMsgBtn = skButton::make(kTabX,       kContentY + 262, 100, 30, "MessageBox")
            ->onClick([msgBox, hwnd]() {
                msgBox->show("Alert", "This is an skMessageBox.\nWarning type with icon.", skMessageType::Warning);
                InvalidateRect(hwnd, nullptr, FALSE);
            });
        auto showPopBtn = skButton::make(kTabX + 108, kContentY + 262, 80, 30, "Popover")
            ->onClick([popover, hwnd, kTabX = kTabX, kContentY = kContentY]() {
                popover->openAt(kTabX + 108 + 40, kContentY + 262);
                InvalidateRect(hwnd, nullptr, FALSE);
            });
        ...
        // Color picker
        auto colorSwatch  = skLabel::make(kTabX + 308, kContentY + 262, 70, 30, "", 11.f);
        auto pickColorBtn = skButton::make(kTabX + 204, kContentY + 262, 96, 30, "Pick Color")
            ->onClick([colorDlg, colorSwatch, hwnd]() {
        ...
        // Date / file row
        auto dateResult = skLabel::make(kTabX + 210, kContentY + 300, kTabW - 210, 30, "No date selected", 11.f);
        auto fileResult = skLabel::make(kTabX,        kContentY + 338, kTabW,       16, "", 10.f);

        auto pickDateBtn = skButton::make(kTabX,       kContentY + 300, 96, 30, "Pick Date")
            ->onClick([datePicker, dateResult, hwnd]() {
                datePicker->show({});
        ...
        auto browseBtn = skButton::make(kTabX + 104, kContentY + 300, 96, 30, "Browse files")
            ->onClick([fileDlg, fileResult, hwnd]() {
                fileDlg->show("C:\\Users\\BAINARI 8136\\skia-dev");
        ...
        // Font dialog row
        auto fontResult  = skLabel::make(kTabX + 104, kContentY + 356, kTabW - 104, 26, "Segoe UI 14pt", 11.f);
        auto pickFontBtn = skButton::make(kTabX,       kContentY + 356, 96, 26, "Pick Font")
            ->onClick([fontDlg, fontResult, hwnd]() {
                ...
                fontDlg->setOnConfirm([fontResult, hwnd](skFontSelection sel) {
                    ...
                    fontResult->setText(info); InvalidateRect(hwnd, nullptr, FALSE);
                });
                InvalidateRect(hwnd, nullptr, FALSE);
            });
    ...
    std::vector<std::shared_ptr<skWidget>> dataW;
    {
        ...
        table->setOnSelect([propGrid, hwnd](int, const std::vector<std::string>& c) {
            if (c.size() >= 3) {
                propGrid->setProperty(0, c[0]); propGrid->setProperty(1, c[1]);
                propGrid->setProperty(2, c[2]); InvalidateRect(hwnd, nullptr, FALSE);
            }
        });
        ...
        auto gridLbl = skLabel::make(kTabX, kContentY + 288, kTabW, 18, "Inline-editable grid", 12.f);
    ...
    std::vector<std::shared_ptr<skWidget>> devW;
    {
        ...
        auto flexRow = skFlexSizer::make(skFlexSizer::Direction::Row, 8)
            ->addFlex(chart,  0.52f)
            ->addFlex(devCon, 0.48f);
        ...
        auto infoBtn = skButton::make(kTabX,       kContentY + 208, 80, 24, "Info")
            ->onClick([devCon, hwnd]() { devCon->info("Info message appended");  InvalidateRect(hwnd, nullptr, FALSE); });
        auto warnBtn = skButton::make(kTabX + 88,  kContentY + 208, 80, 24, "Warn")
            ->onClick([devCon, hwnd]() { devCon->warn("Warning: check this");    InvalidateRect(hwnd, nullptr, FALSE); });
        auto errBtn  = skButton::make(kTabX + 176, kContentY + 208, 80, 24, "Error")
            ->onClick([devCon, hwnd]() { devCon->error("Error: operation failed"); InvalidateRect(hwnd, nullptr, FALSE); });
        auto clrBtn  = skButton::make(kTabX + 264, kContentY + 208, 80, 24, "Clear")
            ->onClick([devCon, hwnd]() { devCon->clear(); InvalidateRect(hwnd, nullptr, FALSE); });
        for (auto& b : { infoBtn, warnBtn, errBtn, clrBtn }) { window->addWidget(b); devW.push_back(b); }

        constexpr int kCvW = 258;
        auto canvasLbl = skLabel::make(kTabX,             kContentY + 240, kCvW,             18, "Draw canvas (drag to paint)", 12.f);
        auto insLbl    = skLabel::make(kTabX + kCvW + 8,  kContentY + 240, kTabW - kCvW - 8, 18, "Theme Inspector", 12.f);
        ...
        cv->setOnPaint([strokes, strokeColors](SkCanvas* canvas, int cw, int ch) {
            SkPaint gridP;
            gridP.setColor(SkColorSetARGB(30, 100, 100, 255));
            gridP.setStyle(SkPaint::kStroke_Style);
            gridP.setStrokeWidth(0.5f);
            ...
            for (int i = 0; i < (int)strokes->size(); ++i) {
                ...
                sp.setStrokeWidth(2.5f);
                sp.setStrokeCap(SkPaint::kRound_Cap);
                sp.setStrokeJoin(SkPaint::kRound_Join);
        ...
        auto clearCvBtn = skButton::make(kTabX, kContentY + 428, 66, 22, "Clear")
            ->onClick([strokes, strokeColors, hwnd]() {
                strokes->clear(); strokeColors->clear(); InvalidateRect(hwnd, nullptr, FALSE);
            });

        for (int ci = 0; ci < 5; ++ci) {
            SkColor pc = palette[ci];
            auto cb = skButton::make(kTabX + 74 + ci * 48, kContentY + 428, 40, 22, "  ")
                ->onClick([penColor, pc, hwnd]() { *penColor = pc; InvalidateRect(hwnd, nullptr, FALSE); });
    ...
    std::vector<std::shared_ptr<skWidget>> mediaW;
    {
        ...
        auto ttBtn = skButton::make(kTabX, kContentY + 198, 110, 24, "Show Tooltip")
            ->onClick([tooltip, hwnd, kTabX = kTabX, kContentY = kContentY]() {
                tooltip->showAt(kTabX + 55, kContentY + 198, "Hello from skTooltip!", 25);
                InvalidateRect(hwnd, nullptr, FALSE);
            });
        ...
        dock->addChild(skLabel::make(0, 0, 0, 0, "Dock panel: Web (left)  \xc2\xb7  PDF (right)", 11.f),
                       skDockPanel::Dock::Top, 20);
    ...
    tabBar->onChange([=](int idx, const std::string&) {
    ...
    auto modal = skModal::make(0, 0, 900, 700)
        ->onConfirm([nameInput, prioInput, window, spinner]() {
            spinner->runFor(20);
    ...
    window->onResize([modal, statusBar, msgBox, colorDlg, datePicker, fileDlg, fontDlg,
                      tooltip, drawer, menuBar](int nw, int nh) {
